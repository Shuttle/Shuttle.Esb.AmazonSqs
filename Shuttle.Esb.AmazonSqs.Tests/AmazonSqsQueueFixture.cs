...
namespace Shuttle.Esb.AmazonSqs.Tests
{
    ...
    public class AmazonSqsQueueFixture : BasicQueueFixture
    {
        ...
        {
            TestSimpleEnqueueAndGetMessage(AmazonSqsFixture.GetServiceCollection(), "amazonsqs://local/{0}");
        ...
        {
            TestReleaseMessage(AmazonSqsFixture.GetServiceCollection(), "amazonsqs://local/{0}");
        }

        [Test]
        public void Should_be_able_to_get_message_again_when_not_acknowledged_before_queue_is_disposed()
        {
            TestUnacknowledgedMessage(AmazonSqsFixture.GetServiceCollection(), "amazonsqs://local/{0}");
        }
    }
}